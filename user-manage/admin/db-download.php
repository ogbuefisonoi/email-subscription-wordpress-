<?php 
    if(isset($_REQUEST['csvstart']) && !empty($_REQUEST['csvstart'])) {
        ob_get_level();
        ob_end_clean();
        exportAsCSV();
        }
    ...
    function exportAsCSV () {
        ...
        global $wpdb;
        $filename = 'user.csv';
        $headers = array('name', 'address', 'website', 'email');
        ...
        foreach($results as $results1)
            ...
            $row = array(
                $results1->user_name,
                $results1->user_address,
                $results1->user_website,
                $results1->user_email
            );
...
<?php 
        ...
        // $exportCSV = new export_table_to_csv('user_manage',';','report');              //Make your changes on these lines
?>